...
\ignorethis{\karen{The point of the paper is to design controller that works in the real world, instead of finding the ground truth about the hardware parameters. This point needs to be made more clear in the introduction.}}
...
\ignorethis{\karen{Regarding Task IV, we will use it as a counter example for the following statement: Multiple training tasks will improve the generalization, but they have to be similar tasks. When using dissimilar training tasks to calibrate the simulator, the results are poor.}}
